...
def predict(self,model='ridge',cv='KFold',folds=5,layers=5,neurons=50,remove_linear_vars=False,remove_cat_vars=False):
	...
	for train, test in model_cv.split(self.subject_measures.subject.values):
		...
		if type(remove_linear_vars) != bool:
			...
			nuisance_model.fit(self.subject_measures[remove_linear_vars].values[train],x_train) #fit the nuisance_model to training data
			x_train = x_train - nuisance_model.predict(self.subject_measures[remove_linear_vars].values[train]) #remove nuisance from training data
			x_test = x_test - nuisance_model.predict(self.subject_measures[remove_linear_vars].values[test]) #remove nuisance from test data
		if type(remove_cat_vars) != bool:
			nuisance_model = LogisticRegression()
			nuisance_model.fit(self.subject_measures[remove_linear_vars].values[train],x_train)
			x_train = x_train - nuisance_model.predict(self.subject_measures[remove_cat_vars].values[train])
			x_test = x_test - nuisance_model.predict(self.subject_measures[remove_cat_vars].values[test])
		if model == 'ridge':
			m = RidgeCV()
		if model == 'deep':
